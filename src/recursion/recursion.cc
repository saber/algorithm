...
using namespace std;
//! \brief 完成 3 个递归实例。电影院问题、爬楼梯问题、最终推荐人问题
//! TODO 调试递归代码，以及参考 github 写成类的形式，封装一起！
namespace glib {
...
_Value FinalRecommender(const glib::KeyMap<_Key, _Value> &record_table, const _Key inquiry) {
    ...
    return FinalRecommender(record_table, inquiry_record->second); // 如何这里用 first 就会出现段错误，也就是一致在这里循环
}
} // namespace glib
